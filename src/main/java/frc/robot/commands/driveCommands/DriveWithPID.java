...
public class DriveWithPID extends CommandBase {
    ...
    public DriveWithPID(double setpointDistance, double epsilonDistance, double setpointYaw, double epsilonYaw) {
        ...
        pidYAxis = new PIDController(0.05, 0, 0.005);
        pidYAxis.setTolerance(epsilonDistance);

        pidZAxis = new PIDController(0.05, 0, 0.005);
    ...
    @Override
    public void execute() {
        drive.holonomicDrive(0.0, MathUtil
                .clamp(pidYAxis.calculate(drive.getAverageForwardEncoderDistance(), setpointDistance), -0.5, 0.5),
